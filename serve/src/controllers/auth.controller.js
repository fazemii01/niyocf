...
async function login(req, res) {
  try {
    ...
    // Make the jwt.sign callback async to use await
    jwt.sign(
      payload,
      ...
      async (err, token) => {
        if (err) {
          // If jwt.sign itself fails, pass to the main catch block
          return res
            .status(500)
            .json({
              status: 500,
        ...
        try {
          ...
          // Await the tokenModel.store operation
          const storeResult = await tokenModel.store({
            token: token,
            expired_at: expirationDate,
          });
          console.log(storeResult.message); // Log success message from store

          // Only send success response after token is stored
          res.status(201).json({
            status: 201,
          ...
          // Catch errors from tokenModel.store or other issues in this block
          console.error(
            "Error during token storage or response sending:",
            storeError
          );
          // Pass to the main catch block or handle specifically
          // For simplicity, let main catch block handle it, or send a specific error
          res
            .status(500)
            .json({
              status: 500,
              msg: "Failed to finalize login",
    ...
    // This outer catch will now also catch errors re-thrown from jwt.sign callback's try-catch
    console.error("Login controller error:", err); // Changed to console.error
    res.status(500).json({
      status: 500,
